...
from script import get_data, utility, earlystopping, report_generator
import pickle
...
def get_parameters():
    ...
    parser = argparse.ArgumentParser(
        description="STGCN - 时空图卷积网络用于能源系统预测"
    )

    # 基础配置
    parser.add_argument(
        "--enable_cuda", type=bool, default=True, help="是否启用CUDA，默认为True"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="随机种子，用于稳定实验结果"
    )
    ...
    parser.add_argument(
        "--need_grad", type=bool, default=False, help="是否需要计算梯度（用于PDE损失）"
    )
    ...
    # 时间序列参数
    parser.add_argument(
        "--n_his", type=int, default=12, help="历史时间步数（输入序列长度）"
    )
    parser.add_argument("--n_pred", type=int, default=1, help="预测时间步数，默认为1")
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="蒸汽节点特征维度（通常为流量G和压力P）",
    )
    parser.add_argument(
        "--electricity_dim", type=int, default=2, help="电力节点特征维度"
    )
    ...
    parser.add_argument(
        "--enable_bias", type=bool, default=True, help="是否启用偏置项，默认为True"
    )
    parser.add_argument("--droprate", type=float, default=0.05, help="Dropout比率")
    parser.add_argument(
        "--pregraph_conv_type",
        type=str,
        default="preGraphLayer_nomid",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="边权重类型：1-无权重，2-乘法，3-除法",
    )

    # 训练参数
    parser.add_argument("--lr", type=float, default=0.001, help="学习率")
    parser.add_argument(
        "--weight_decay_rate", type=float, default=0.1, help="权重衰减率（L2正则化）"
    )
    parser.add_argument("--batch_size", type=int, default=128, help="批次大小")
    parser.add_argument("--epochs", type=int, default=40, help="训练轮数，默认为40")
    parser.add_argument(
        "--opt", type=str, default="adam", help="优化器类型：adam, rmsprop, adamw"
    )
    ...
    parser.add_argument("--patience", type=int, default=10, help="早停机制的耐心值")
    parser.add_argument(
        "--save_interval", type=int, default=5, help="模型保存间隔（epoch）"
    )

    # 物理参数（用于PDE损失计算）
    parser.add_argument("--lam", type=float, default=0.05, help="摩擦系数")
    parser.add_argument(
        "--Tb", type=float, default=549.9658495560284, help="基准温度（K）"
    )
    parser.add_argument("--Ta", type=float, default=288.15, help="空气温度（K）")
    parser.add_argument(
        "--R", type=float, default=0.4615, help="蒸汽气体常数（kJ/(kg·K)）"
    )
    parser.add_argument(
        "--R_air", type=float, default=0.287, help="空气气体常数（kJ/(kg·K)）"
    )
...
def get_iter(G_df, P_df, len_train, len_val):
    ...
    # 将时间序列数据转换为滑动窗口格式
    x_train_G, y_train_G = get_data.data_transform(
        train_G, args.n_his, args.n_pred, device, args
    )
    x_val_G, y_val_G = get_data.data_transform(
        val_G, args.n_his, args.n_pred, device, args
    )
    x_test_G, y_test_G = get_data.data_transform(
        test_G, args.n_his, args.n_pred, device, args
    )
    x_train_P, y_train_P = get_data.data_transform(
        train_P, args.n_his, args.n_pred, device, args
    )
    x_val_P, y_val_P = get_data.data_transform(
        val_P, args.n_his, args.n_pred, device, args
    )
    x_test_P, y_test_P = get_data.data_transform(
        test_P, args.n_his, args.n_pred, device, args
    )
...
def load_data(args, data_path, Sscore_path, Escore_path):
    ...
    # 将数据转换为图数据格式
    data_train = get_data.data_split(
        S_x_train, S_y_train, E_x_train, E_y_train, edge_index, S1, S2, E1, E2, t
    )
    data_val = get_data.data_split(
        S_x_val, S_y_val, E_x_val, E_y_val, edge_index, S1, S2, E1, E2, t
    )
    data_test = get_data.data_split(
        S_x_test, S_y_test, E_x_test, E_y_test, edge_index, S1, S2, E1, E2, t
    )

    # 创建数据加载器
    # 使用自定义的collate函数来处理图数据批次
    train_iter = DataLoader[Any](
        data_train,
        batch_size=args.batch_size,
        shuffle=True,
        collate_fn=utility.custom_collate,
    )  # 训练集数据加载器，batch_size为128，打乱顺序，使用自定义的collate函数来处理图数据批次
    val_iter = DataLoader[Any](
        data_val,
        batch_size=args.batch_size,
        shuffle=True,
        collate_fn=utility.custom_collate,
    )
    test_iter = DataLoader[Any](
        data_test,
        batch_size=args.batch_size,
        shuffle=False,
        collate_fn=utility.custom_collate,
...
def train(
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_iter, desc="Training"):
            # 从批次中提取数据
            t = batch.t  # 时间信息，[batch_size, 12, 31]
            xs = batch.xs  # 蒸汽节点特征，[batch_size, 2, 12, 24]
            xe = batch.xe  # 电力节点特征，[batch_size, 2, 12, 7]
            edge_index = batch.edge_index  # 图的边索引，[2, 30]
            S1 = batch.S1  # 蒸汽管道长度（直径），[23]
            S2 = batch.S2  # 蒸汽管道直径，[23]
            E1 = batch.E1  # 电力管道长度（直径），[6]
            E2 = batch.E2  # 电力管道直径，[6]
            ys = batch.ys  # 蒸汽标签，[batch_size, 2, 24]
            ye = batch.ye  # 电力标签，[batch_size, 2, 7]
            y = torch.cat((ys, ye), dim=-1)  # 拼接蒸汽和电力标签，[batch_size, 2, 31]
            # 前向传播
            y_pred = model(
                xs, xe, t, edge_index, S1, S2, E1, E2, False
            )  # [batch_size, 2, 1, 31]
            y_pred = y_pred.squeeze(2)  # 移除时间维度 [batch_size, 2, num_nodes]

            # 选择需要评估的节点索引
            indices = torch.tensor(
                [0, 3, 4, 6, 10, 12, 14, 17, 19, 21, 23, 24, 25, 28, 29, 30],
                device=device,
            )
            ...
            if args.need_grad:
                ...
                y = torch.index_select(
                    y.reshape(-1, args.n_vertex), 1, indices
                ).reshape(y.shape[0], 2, len(indices))
                y_pred = torch.index_select(
                    y_pred.reshape(-1, args.n_vertex), 1, indices
                ).reshape(y_pred.shape[0], 2, len(indices))
                ...
                # 总损失 = PDE损失（缩放后）+ 预测损失
                l = (
                    l_grad_steam[0] / 1000000
                    + l_grad_steam[1] / 10000
                    + l_grad_air[0] / 1000000
                    + l_grad_air[1] / 10000
                    + l_p
                )
            else:
        ...
        # 打印训练信息
        train_loss_avg = l_sum / n
        current_lr = optimizer.param_groups[0]["lr"]
        print(
            "Epoch: {:03d} | Lr: {:.20f} | Train loss: {:.6f} | Val loss: {:.6f} | GPU占用: {:.6f} MiB".format(
                epoch + 1, current_lr, train_loss_avg, val_loss, gpu_mem_alloc
            )
        )

        # 记录到报告生成器
        if report_gen is not None:
            report_gen.record_epoch(
                epoch,
                train_loss_avg,
        ...
        # 定期保存模型至报告目录下的models/子文件夹
        if (epoch + 1) % args.save_interval == 0:
            # 确保目标文件夹存在
            model_dir = (
                os.path.join(report_gen.run_dir, "models")
                if report_gen is not None
                else "models"
            )
            os.makedirs(model_dir, exist_ok=True)
            save_path = os.path.join(model_dir, f"model_nofno_notime_{epoch+1}.pth")
            torch.save(model.state_dict(), save_path)
            print(f"模型已保存至 {save_path}")
            if report_gen is not None:
                report_gen.add_saved_model(save_path)
...
def val(model, val_iter, loss):
    ...
    for batch in val_iter:
        ...
        y_pred = model(
            xs, xe, t, edge_index, S1, S2, E1, E2, False
        )  # [batch_size, 2, 1, num_nodes]
        y_pred = y_pred.squeeze(2)  # [batch_size, 2, num_nodes]

        # 选择需要评估的节点索引
        indices = torch.tensor(
            [0, 3, 4, 6, 10, 12, 14, 17, 19, 21, 23, 24, 25, 28, 29, 30], device=device
        )
...
@torch.no_grad()
def test(S_score, E_score, loss, model, test_iter, args, report_gen=None):
    """
    在测试集上评估模型并打印详细指标
    ...
    (
        ...
        air_MAPE_P,
        air_WMAPE_P,
    ) = utility.evaluate_metric(model, test_iter, S_score, E_score, args)

    # 打印测试结果
    print(
        f"测试损失: {test_MSE:.6f} \n"
        f"蒸汽 - 流量G: MAE={steam_MAE_G:.6f} | RMSE={steam_RMSE_G:.6f} | MAPE={steam_MAPE_G:.6f}% | WMAPE={steam_WMAPE_G:.8f}\n"
        f"蒸汽 - 压力P: MAE={steam_MAE_P:.6f} | RMSE={steam_RMSE_P:.6f} | MAPE={steam_MAPE_P:.6f}% | WMAPE={steam_WMAPE_P:.8f}\n"
        f"空气 - 流量G: MAE={air_MAE_G:.6f} | RMSE={air_RMSE_G:.6f} | MAPE={air_MAPE_G:.6f}% | WMAPE={air_WMAPE_G:.8f}\n"
        f"空气 - 压力P: MAE={air_MAE_P:.6f} | RMSE={air_RMSE_P:.6f} | MAPE={air_MAPE_P:.6f}% | WMAPE={air_WMAPE_P:.8f}"
    )

    # 记录测试结果到报告生成器
    if report_gen is not None:
        metrics = {
            "steam_MAE_G": steam_MAE_G,
            "steam_RMSE_G": steam_RMSE_G,
            "steam_MAPE_G": steam_MAPE_G,
            "steam_WMAPE_G": steam_WMAPE_G,
            "steam_MAE_P": steam_MAE_P,
            "steam_RMSE_P": steam_RMSE_P,
            "steam_MAPE_P": steam_MAPE_P,
            "steam_WMAPE_P": steam_WMAPE_P,
            "air_MAE_G": air_MAE_G,
            "air_RMSE_G": air_RMSE_G,
            "air_MAPE_G": air_MAPE_G,
            "air_WMAPE_G": air_WMAPE_G,
            "air_MAE_P": air_MAE_P,
            "air_RMSE_P": air_RMSE_P,
            "air_MAPE_P": air_MAPE_P,
            "air_WMAPE_P": air_WMAPE_P,
        }
        report_gen.set_test_results(test_MSE, metrics)
...
if __name__ == "__main__":
    ...
    # 记录模型信息到报告生成器
    report_gen.set_model_info(model, args, blocks)
    ...
    # 在测试集上评估模型
    test(S_score, E_score, loss, model, test_iter, args, report_gen)
