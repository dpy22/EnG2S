...
class NodeEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[-1]):
            # 提取第i个节点的数据 [batch_size, feature_dim, time_steps]
            node_data = x[..., i]
            # 调整维度顺序为 [batch_size, time_steps, feature_dim]
            node_data = node_data.permute(0, 2, 1)
            # 通过FNO进行特征提取
            node_output = self.fno(node_data)
            # 恢复维度顺序为 [batch_size, embed_dim, time_steps]
...
class edge_embed(nn.Module):
    ...
    def forward(self, weight1, weight2, ratio):
        ...
        参数:
            weight1: 第一个边属性（如管道长度或直径）
            weight2: 第二个边属性（如管道直径）
            ratio: 缩放比例因子
        ...
        return torch.squeeze(x, 0)  # [num_edges]
